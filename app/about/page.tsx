...
const DIFFERENTIATORS = [
    {
        ...
        description:
            'We measure success by business results — revenue, time saved, errors reduced — not by the AI tools we deploy.',
    },
    {
        icon: (
            <svg className="w-5 h-5" fill="none" stroke="currentColor" viewBox="0 0 24 24" aria-hidden="true">
        ...
        description:
            "We move fast because we've built this before. Our frameworks compress months of trial-and-error into weeks of focused execution.",
    },
    {
        icon: (
            <svg className="w-5 h-5" fill="none" stroke="currentColor" viewBox="0 0 24 24" aria-hidden="true">
        ...
        description:
            "If AI isn't the right solution for your problem, we'll tell you. We'd rather lose a deal than waste your budget.",
    },
    {
        icon: (
            <svg className="w-5 h-5" fill="none" stroke="currentColor" viewBox="0 0 24 24" aria-hidden="true">
        ...
        title: 'SMB-native',
        description:
            "We don't adapt enterprise playbooks for small teams. We build for the constraints, budgets, and pace of growing businesses.",
    },
] as const
...
export default function AboutPage() {
    return (
        <>
            ...
            >
                ...
                <div className="relative z-10 mx-auto max-w-4xl px-4 sm:px-6 lg:px-8 text-center">
                    ...
                    <p className="text-text-muted text-lg sm:text-xl max-w-2xl mx-auto leading-relaxed font-body">
                        No buzzwords. No AI theatre. Just practitioners who build things that work — and stick around until the numbers prove it.
                    </p>
                </div>
            </section>

            <SectionWrapper className="border-t border-border-subtle" aria-label="Who we are">
                <div className="grid grid-cols-1 lg:grid-cols-2 gap-12 lg:gap-20 items-center">
                    <div>
                        ...
                        <div className="mt-8 grid grid-cols-1 sm:grid-cols-3 gap-3">
                            {[
                                { role: 'Strategy Lead', focus: 'ROI and roadmap clarity' },
                                { role: 'AI Systems Engineer', focus: 'Production-ready delivery' },
                                { role: 'Workflow Architect', focus: 'Cross-system automation' },
                            ].map(({ role, focus }) => (
                                <Card key={role} className="p-4 border border-border-subtle bg-surface/50">
                                    <p className="font-display font-semibold text-sm text-text-primary mb-1">{role}</p>
                                    <p className="text-xs font-body text-text-muted">{focus}</p>
                                </Card>
                            ))}
                        </div>
                    </div>
                    ...
                    <div className="grid grid-cols-2 gap-4">
                        {[
                            { value: '12+', label: 'Industries in production', color: 'text-primary' },
                            { value: 'Week 1', label: 'First outcomes delivered', color: 'text-secondary' },
            ...
            <SectionWrapper className="bg-surface border-y border-border-subtle" aria-label="Our philosophy">
                <div className="text-center mb-14">
                    <p className="text-xs font-body font-semibold tracking-widest uppercase text-primary mb-4">
                        How We Think
                    </p>
                    <h2 className="font-display font-bold text-3xl sm:text-4xl md:text-5xl text-text-primary tracking-tight">
                        Experimentation. <span className="text-gradient-orange">Execution.</span> Scale.
                    </h2>
                </div>
